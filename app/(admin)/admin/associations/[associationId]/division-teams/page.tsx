...
import DivisionTeamOverviewClient from "@/components/admin/associations/DivisionTeamOverviewClient";
...
export default async function AssociationDivisionTeamsPage({
  params,
}: {
  ...
  if (session.role !== "super-admin" && sessionAssoc && sessionAssoc !== associationId) {
    redirect(`/admin/associations/${encodeURIComponent(sessionAssoc)}/division-teams`);
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap items-center gap-2 text-sm">
        ...
        <span className="text-slate-400">/</span>
        <span className="font-bold text-[#06054e]">Teams &amp; divisions</span>
      </div>
...
export async function generateMetadata({
  ...
  const { associationId } = await params;
  return {
    title: `Teams & divisions | ${associationId} | Admin`,
  };
}
