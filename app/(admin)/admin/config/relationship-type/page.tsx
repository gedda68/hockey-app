// app/(admin)/admin/config/relationship-type/page.tsx
"use client";
...
export default function RelationshipTypeConfigPage() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      toast.error("Failed to load relationship type options");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAdd = async (data: any) => {
    console.log("➕ Creating relationship type:", data);
  ...
  const handleEdit = async (id: string, data: any) => {
    console.log("✏️ Editing relationship type ID:", id);
  ...
  const handleDelete = async (id: string, name: string) => {
    console.log("🗑️ Deleting relationship type ID:", id);
    const res = await fetch(`/api/admin/config/relationship-type?id=${id}`, {
      method: "DELETE",
    });
  ...
  const handleToggleActive = async (item: ConfigItem) => {
    ...
    toast.success(
      `Relationship type ${!item.isActive ? "activated" : "deactivated"}`,
    );
  };
  ...
  return (
    <ConfigTable
      ...
      title="Relationship Type Options"
      icon={UserCircle}
      singularName="Relationship Type Option"
      fields={[
        {
          name: "name",
          label: "Name",
