...
// D4 — Rep tournament pool fixtures: generate from draw, schedule, enter results.
  ...
  async function generateFixtures() {
    ...
    try {
      const r = await fetch(`${apiBase}/fixtures`, {
        ...
        body: JSON.stringify({ replace: genReplace, doubleRound: genDouble }),
      });
      const j = await r.json();
      if (!r.ok) {
      ...
      setMsg(`Generated ${j.created ?? 0} pool fixture(s).`);
      setFixtures(j.fixtures ?? []);
    } catch {
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="rounded-2xl border-2 border-slate-100 bg-slate-50/80 p-4 mb-8 space-y-3">
        <p className="text-xs font-black uppercase text-slate-500">
          Generate pool round-robin (D4)
        </p>
        <p className="text-xs text-slate-600">
          Uses pools on the tournament draw (D3). Each pool needs at least two entries. Existing
          fixtures block generation unless you replace.
        </p>
        ...
        >
          {busy ? <Loader2 className="animate-spin inline" size={14} /> : null} Generate from draw
        </button>
      </div>
        ...
        <div className="overflow-x-auto rounded-2xl border border-slate-200">
          <table className="w-full text-sm">
            ...
            <tbody>
              {fixtures.map((f) => (
                <FixtureRow
                  key={`${f.fixtureId}-${f.resultStatus ?? ""}-${f.result?.homeScore ?? ""}-${f.result?.awayScore ?? ""}`}
                  f={f}
                  busy={busy}
...
function FixtureRow({
  ...
  return (
    <tr className="border-t border-slate-100 hover:bg-slate-50/80">
      <td className="px-3 py-2 font-medium">{f.poolLabel ?? "—"}</td>
      <td className="px-3 py-2">{f.poolRound ?? "—"}</td>
