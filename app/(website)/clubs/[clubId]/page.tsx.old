...
// Individual club detail page using slug from club name
...
// Get single club from database by slug
async function getClubBySlug(slug: string): Promise<Club | null> {
  ...
  try {
    ...
    // Get all active clubs and find by generated slug
...
async function getAllClubSlugs(): Promise<string[]> {
  const client = new MongoClient(process.env.MONGODB_URI!);
  ...
  try {
    ...
    return clubs.map((club) => generateSlug(club.name as string));
  } catch (error) {
    console.error("Error fetching club slugs:", error);
    return [];
  } finally {
    await client.close();
  }
...
export async function generateMetadata({
  ...
  return {
    title: `${club.name} | Hockey Clubs`,
    description: club.description || `Learn more about ${club.name}`,
  };
}
...
export default async function ClubDetailsPage({ params }: { params: Params }) {
  ...
  return (
    <div
      ...
      style={{ "--club-color": club.colors.primary } as React.CSSProperties}
    >
      ...
      >
        <div className="max-w-4xl mx-auto px-4 flex flex-col items-center text-center">
          ...
          {club.description && (
            <div
              className="text-xl opacity-90 mt-4 max-w-2xl"
        ...
        <div className="bg-white rounded-3xl shadow-xl -mt-20 border-2 border-slate-200 p-8">
          ...
          {club.established && (
            <div className="mb-6">
              ...
              <p className="text-slate-700">{club.established}</p>
            </div>
          )}
          ...
          {(club.address.street || club.address.suburb) && (
            <div className="mb-6">
              ...
              <div className="text-slate-700">
                {club.homeGround && (
                  <p className="font-semibold">{club.homeGround}</p>
                )}
