...
// 5-level system
export const LEVEL_MAP: Record<number, { label: string; color: string; short: string }> = {
  1: { label: "National",     short: "L1", color: "bg-purple-100 text-purple-700 border-purple-300" },
  2: { label: "Sub-national", short: "L2", color: "bg-indigo-100 text-indigo-700 border-indigo-300" },
  3: { label: "State",        short: "L3", color: "bg-blue-100   text-blue-700   border-blue-300"   },
  4: { label: "Regional",     short: "L4", color: "bg-teal-100   text-teal-700   border-teal-300"   },
  5: { label: "City",         short: "L5", color: "bg-green-100  text-green-700  border-green-300"  },
};
...
function DeleteModal({ association, onConfirm, onCancel, isDeleting }: DeleteModalProps) {
  return (
        ...
        {(association.childrenCount > 0 || association.clubsCount > 0) && (
          <div className="mt-3 p-3 bg-yellow-50 border-2 border-yellow-200 rounded-xl">
            ...
            <p className="text-sm font-bold text-yellow-700">
              This association has {association.childrenCount} child association(s) and{" "}
              {association.clubsCount} club(s). These relationships must be removed first.
            </p>
          </div>
        )}
...
export default function AssociationsList() {
  ...
  const fetchAssociations = useCallback(async () => {
    ...
    try {
      ...
      const associationsArray = Array.isArray(data) ? data : data.associations || [];
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/associations/${deleteTarget.associationId}`, {
        method: "DELETE",
      });
  ...
  const getLevelBadge = (level: number) => {
    ...
    return (
      <span className={`px-2 py-1 rounded text-xs font-black border-2 whitespace-nowrap ${info.color}`}>
        {info.label}
      </span>
    );
  };
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-[2rem] shadow-xl border border-slate-100 p-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-black text-[#06054e]">Associations</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <select
              ...
              onChange={(e) =>
                setLevelFilter(e.target.value === "" ? null : parseInt(e.target.value))
              }
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-5">
            <div className="text-xs font-black uppercase text-slate-500 mb-1">Total</div>
            <div className="text-3xl font-black text-[#06054e]">{associations.length}</div>
          </div>
          {[1, 2, 3, 4, 5].map((lvl) => {
            const info = LEVEL_MAP[lvl];
            const count = associations.filter((a) => a.level === lvl).length;
            return (
              <div key={lvl} className="bg-white rounded-2xl shadow-xl border border-slate-100 p-5">
                <div className="text-xs font-black uppercase text-slate-500 mb-1">{info.label}</div>
                <div className={`text-3xl font-black ${info.color.split(" ")[1]}`}>{count}</div>
              </div>
            );
          })}
        </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden">
          {filteredAssociations.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-xl font-bold text-slate-500">No associations found</p>
              <Link
                href="/admin/associations/new"
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-50 border-b-2 border-slate-200">
                    <th className="px-6 py-4 text-left text-sm font-black uppercase text-slate-600">Association</th>
                    <th className="px-6 py-4 text-left text-sm font-black uppercase text-slate-600">Code</th>
                    <th className="px-6 py-4 text-left text-sm font-black uppercase text-slate-600">Level</th>
                    <th className="px-6 py-4 text-left text-sm font-black uppercase text-slate-600">Parent</th>
                    <th className="px-6 py-4 text-left text-sm font-black uppercase text-slate-600">Children</th>
                    <th className="px-6 py-4 text-left text-sm font-black uppercase text-slate-600">Clubs</th>
                    <th className="px-6 py-4 text-left text-sm font-black uppercase text-slate-600">Status</th>
                    <th className="px-6 py-4 text-left text-sm font-black uppercase text-slate-600">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredAssociations.map((assoc) => (
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="font-bold text-slate-900">{assoc.name}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="font-black text-[#06054e]">{assoc.code}</div>
                      </td>
                      <td className="px-6 py-4">{getLevelBadge(assoc.level)}</td>
                      <td className="px-6 py-4">
                        {assoc.parent ? (
                          <div className="text-sm font-bold text-slate-600">{assoc.parent.code}</div>
                        ) : (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-slate-700">{assoc.childrenCount}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <Users size={16} className="text-slate-400" />
                          <span className="font-bold text-slate-700">{assoc.clubsCount}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">{getStatusBadge(assoc.status)}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <Link
