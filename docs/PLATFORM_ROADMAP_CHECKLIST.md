...
- [X] **D5** Avoid duplicate team records; link tournament entries to canonical team/club entities. `team_tournament_entries.canonicalTeamId` (from `teams.canonicalTeamId ?? teamId` on POST); `POST /api/admin/team-tournaments` returns **409** `duplicate_canonical_team_entry` if another non-withdrawn entry shares the same canonical identity; `PUT .../team-tournaments/[entryId]` with `{ "syncFromTeam": true }` refreshes denormalised team fields + canonical id; helpers `lib/tournaments/teamTournamentCanonical.ts`; admin team-tournaments UI shows ids + sync; tests `teamTournamentCanonical.test.ts`.
...
- [X] **F1** Official register (baseline): Mongo `association_official_register`; `GET/POST .../official-register`, `PATCH/DELETE .../official-register/[recordId]` (`association.fees`); UI `/admin/associations/[associationId]/official-register`. Used to resolve `umpireId` → display name (plus member-id fallback in `lib/officiating/resolveUmpireDisplay.ts`). **Follow-up shipped:** `homeRegion`, `nationalRegisterId` (placeholder format in schema), `POST .../official-register/import` bulk JSON, admin UI fields + import.
...
- [X] **F3** Allocation workflow (schema baseline): `league_fixtures.umpires[]` supports `allocationStatus` (`assigned` | `accepted` | `declined`), `dateDeclined`, `dateNotified` (`lib/db/schemas/leagueFixture.schema.ts`); set via existing fixture `PATCH`. **Follow-up shipped:** optional `notifyAssignedUmpires` on fixture PATCH + Resend (`lib/officiating/umpireAssignmentNotify.ts`), `isStandby` on slots, `GET/PATCH /api/member/my-umpire-assignments`, `/my-umpiring` page.
...
- [X] **F5** Reporting (baseline): `GET .../associations/[associationId]/officiating-report?seasonCompetitionId=` (`association.fees` or `competitions.manage`); fixture slot counts by allocation status, honoraria counts by status, top umpires by fixture coverage; UI `/admin/associations/[associationId]/officiating-report`. **Follow-up shipped:** `dateFrom` / `dateTo` on `scheduledStart`, `byClub` / `byRegion` (register), `format=csv`, allocation bar + breakdown tables in UI.
...
- [X] **G3** Public-safe display vs private contact details (especially juniors). `showEmailOnPublicSite` / `showPhoneOnPublicSite` on `teamRosters` staff; `GET /api/public/club-team-rosters?clubId=&season=` (middleware public) returns staff with email/phone only when opted in (from linked member contact). `GET /api/rosters` sanitizes legacy `teams[].staff` person objects the same way; `/api/rosters` allowlisted for unauthenticated representative page. Admin **Add/Edit staff** modal: WWCC fields + public contact checkboxes + `staffRoleCode`; qualifications accept legacy object array via schema preprocess.
