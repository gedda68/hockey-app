...
- [X] **E6** Player stats: `league_fixtures.matchEvents[]` (`FixtureMatchEventSchema` — goal / green_card / yellow_card / red_card; optional `assistMemberId` on goals). Admin `PATCH /api/admin/season-competitions/[seasonCompetitionId]/fixtures/[fixtureId]/match-events` (`results.manage`, replaces array, validates teamId ∈ {home, away}). Public `GET /api/competitions/player-stats?seasonCompetitionId=&memberId=&limit=` (leaderboard + optional per-player fixture breakdown); `GET /api/fixtures` and `lib/data/matches.ts` expose `matchEvents` when the fixture result is public. Aggregation: `lib/competitions/playerSeasonStats.ts`.
