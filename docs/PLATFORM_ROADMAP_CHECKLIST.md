...
- [X] **D3** Draw formats: pools, knockout, cross-pool finals; manual seeding + optional import from standings. Implemented: `rep_tournaments.draw` (`TournamentDrawStateSchema`), `lib/tournaments/tournamentDraw.ts` (snake pools, single-elim R1, cross-pool winners, standings seed import), `GET/PUT /api/admin/tournaments/[id]/draw`, `POST .../draw/generate`, `POST .../draw/import-seeds`; admin tournament modal section “Tournament draw (D3)”; tests `tournamentDraw.test.ts`.
