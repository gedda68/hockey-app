...
- [x] **B2** **Video and rich media** — Highlights, embeds, club channels. *Shipped (partial):* optional `videoUrl` on `news` items (admin create/edit supports it); public modals render allowlisted embeds (YouTube/Vimeo) via `lib/website/videoEmbeds.ts`. *Follow-up:* oEmbed + curated playlists per tenant.
- [x] **B3** **Identity / logged-in fan** — Tips, favourites, alerts. **Shipped:** `members.fanPreferences` (`lib/member/fanPreferences.ts`) — `followedTeamIds`, `favouriteClubIds`, fixture-change **email** + **push** toggles, `pushSubscriptions` (cap), weekly tips + match-day tips. **API:** `GET`/`PUT` `/api/member/fan-preferences` (club teams + **association clubs** for favourites); `GET`/`POST` `/api/member/push/vapid-public-key`, `/api/member/push/subscribe`, `/api/member/push/unsubscribe`. **Service worker:** `public/competitions/fan-sw.js` (scope `/competitions/`). **My fixtures:** followed teams merged in `getMyTeamIdsForSession`. **Epic J1:** admin fixture `notifyScheduleChange` runs when **published** + schedule/venue changed — **emails** to admin list ∪ fan followers (`fanFixtureFollowers.ts`); **push** to opted-in subscribers (`fanFixturePushNotify.ts`). Admin `scheduleChangeNotifyEmails` may be **empty** (fans-only). **Cron:** `GET /api/cron/weekly-fan-tips` + `CRON_SECRET` (`weeklyFanTipsDigest.ts`). **UI:** `FanPreferencesPanel` on `/competitions/my-fixtures`. **Env:** `NEXT_PUBLIC_VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `WEB_PUSH_CONTACT`, `CRON_SECRET`.
- [x] **B4** **Pathways narrative** — Clear “Play”, “Coach”, “Umpire”, “Volunteer” entry points per **tenant** (not only global). **Shipped:** `lib/website/pathwaysCards.ts` + `components/website/pathways/PathwaysGrid.tsx` — tenant-aware links (platform / association / club) to **club registration**, **nominations / rep hub**, and **`/login?callbackUrl=…/admin/my-registrations?role=…`** (optional `clubId` / `associationId`). **`/play`** — server page with portal resolution + pathways grid. **Hubs:** `#pathways` on `AssociationHubView` + `ClubHubView`. **Tenant home:** pathways strip on association portal (`app/(website)/page.tsx`). **My registrations:** reads `?role=` / `clubId` / `associationId` after login, opens submit modal; `SubmitModal` keeps prefilled scope when clubs/associations load (`app/(admin)/admin/my-registrations/page.tsx`). **Quick links:** “Get involved” → `/play`.
- [x] **B5** **Sponsors & partners** — Footer and dedicated partners strip per portal using existing branding config. **Shipped:** `PublicTenantPayload.partners` + `normalizePublicPartners` (`lib/tenant/tenantPartners.ts`); association **`branding.partners`** (optional `branding.sponsors` alias) and club **`publicPartners`** / `branding.partners` read in `resolveTenantByPortalSlug`. **Admin:** Association branding tab — partner rows with **Upload logo** → `POST /api/admin/upload/image` category `associations/{id}/partners`; website URL field (public site opens in **new tab**). **Club admin:** Colors section → `ClubPartnersEditor` + **`publicPartners`** on club save; uploads use `clubs/{clubId}/partners` with **`requireResourceAccess`** (`app/api/admin/upload/image/route.ts`). **Footer / home** unchanged from prior B5 ship.
- [x] **B6** **Accessibility & performance** — WCAG contrast, keyboard nav on drawers, LCP for hero images, `prefers-reduced-motion` for carousels. **Shipped:** `lib/a11y/useFocusTrap.ts` + `usePrefersReducedMotion.ts`; **ClubsDrawer** — `role="dialog"`, `aria-labelledby`, Escape + focus trap, Framer `useReducedMotion` for panel/backdrop motion; **TopNavbar** mobile drawer — same pattern, `inert` when closed, `aria-expanded` / `aria-controls` on menu button, higher-contrast mobile link colours + focus rings; **body scroll** locked for mobile menu or clubs drawer; **HomeHeroGallery** — first slide `fetchPriority="high"`, meaningful `alt`, no auto-advance + instant scroll when reduced motion, optional keyboard arrows/Home/End on focused region; **globals.css** — marquee animation off under `prefers-reduced-motion`.
- [x] **B7** **SEO + sharing** — Per-tenant sitemaps, canonical URLs on subdomains, consistent `og:image` per article/club. **Shipped:** `app/sitemap.ts` (URLs from `requestMetadataBase` + `loadPublicTenantFromIncomingHost` — static public routes, scoped news URLs, club hub path for club tenants, apex club directory URLs) + `app/robots.ts` (`sitemap` on same host). **`lib/seo/absolutizeMediaUrl.ts`** + **`lib/seo/plainTextExcerpt.ts`**. **Canonical + OG:** home `app/(website)/page.tsx`, **`/news`** + **`/news/[articleId]`** (shareable article page with hero + `NewsAttachmentsBlocks` + sanitized body; **`getPublicNewsItemById`** in `lib/data/publicNews.ts`), **club layout** (`loadClubPublicDocumentByUrlKey`, logo/icon OG), **association hub** metadata. **Root `app/layout.tsx`:** default Open Graph / Twitter images (tenant favicon or BHA). **Home news** links to article URLs (`HomeNewsAside`, news index).
