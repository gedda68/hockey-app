...
- [ ] **P1** **Association hub content sections** — On `/associations/[id]`, group: Rep & pathways | Championships | Local league | News (scoped) | Contacts — driven by level and configured features. *Shipped (partial):* added **Match day** block (links into `/competitions/this-round` scoped to the association’s first league when available) and a **Member clubs** section (active clubs under the association).
