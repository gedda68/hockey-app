...
- [ ] **P1** **Association hub content sections** — On `/associations/[id]`, group: Rep & pathways | Championships | Local league | News (scoped) | Contacts — driven by level and configured features. *Shipped (partial):* **On-page nav** (`#rep-pathways`, `#championships`, `#local-league`, `#news`, `#contacts`); **Rep & pathways** (level-based intro via `lib/website/associationHubCopy.ts`, CTAs, uncrowned tournament spotlights); **Championships & rep events** (full tournament list, champions surfaced); **Local league** (My fixtures, this round / leagues, member clubs, season competitions); **News** (tenant-scoped `getPublicNewsItems` + explainer); **Contacts** (public positions). *Follow-up:* richer “pathways” links from CMS/config; split tournament taxonomy when the model supports it.
