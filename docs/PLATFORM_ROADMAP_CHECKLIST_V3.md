...
- [ ] **N1** **League competition wizard (admin)** — Single guided flow under association scope: create or pick `Competition` → create `SeasonCompetition` for season → edit divisions (reuse season-competition PATCH) → **generate** round-robin (call existing `POST .../fixtures/generate` with clear replace warning + feature flag) → table/grid review → bulk venue/time tools → publish checklist. Entry point: e.g. `/admin/associations/[associationId]/competitions` or `/admin/competitions?associationId=`.
