...
- [x] **O2** **Communications hub** — **Shipped (association MVP):** `communication_hub_settings` + **GET/PATCH** `/api/admin/associations/[id]/communications` (`system.manage` | `association.settings` | `competitions.fixtures` | `association.edit` + scope); **`/admin/associations/[id]/communications`** (fixture email supplement, push topic toggles incl. `fixture_changes`, weekly digest fields reserved); fixture **PATCH** prepends supplement HTML/plain and gates fan push on `fixture_changes`. _Follow-up:_ club scope, weekly digest / news cron, richer templates.
- [x] **O3** **Volunteer roster** — **Shipped (club MVP):** Mongo **`club_volunteer_duty_leads`**; public **`/clubs/[slug]/volunteer-duties`** + **`POST /api/public/clubs/[clubId]/volunteer-duty-interest`** (honeypot); **pathways** volunteer card links to duty roster (club direct, association/platform → club directory); admin **`/admin/clubs/[id]/volunteer-duty-roster`** + **GET/POST** + **PATCH/DELETE** lead (`club.edit` \| `club.members`); **`logPlatformAudit`** `volunteer_duty_roster`; club hub nav + edit-club shortcut. _Follow-up:_ fixture-linked shifts, member merge, association-wide rollup.
- [x] **O4** **Sponsors analytics** — **Shipped:** Mongo **`partner_strip_click_daily`** (UTC day, `$inc` only); **`POST /api/public/partner-strip-click`** validates **`scopeType` + `scopeId` + slot** against live **`normalizePublicPartners`** list (no client-trusted partner ids); **`navigator.sendBeacon`** (+ fetch `keepalive` fallback) on **`onPointerDown`** for outbound links in **`PartnerStripClickable`** wired into **footer** + **association home sidebar**; admin **`GET .../partner-click-stats`** + **`/admin/associations/[id]/partner-analytics`** & **`/admin/clubs/[id]/partner-analytics`** (`association.settings` \| `association.edit` / `club.edit` \| `club.members`). **Indexes:** `scripts/setup-indexes.ts`. _Follow-up:_ optional salted IP hash rate cap; CSV export.
...
- [x] **Q3** **Admin observability** — **Shipped:** JSON line structured logs for admin heavy ops/mutations (`lib/observability/adminTelemetry.ts`) wired into `GET/POST /api/admin/competitions`, `PATCH /api/admin/season-competitions/[id]`, and `POST /api/admin/season-competitions/[id]/fixtures/generate`, correlating `seasonCompetitionId`, `owningAssociationId`, and the acting `userId`/role.
...
5. **O1** _(shipped)_ **/ O5** _(ICS hub shipped; PDF optional)_ **/ B8** — Fan and partner value; deep links long-term.
