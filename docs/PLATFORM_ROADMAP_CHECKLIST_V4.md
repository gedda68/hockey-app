...
| Category | Key gaps |
|----------|----------|
| **Security** | ~~`/api/clubs` and `/api/payments` unauthenticated; dev test routes exposed~~ ✅ Fixed (S1–S4). Remaining: no login rate limiting (S5), cookie flag audit (S6), rich-text XSS (S7) |
