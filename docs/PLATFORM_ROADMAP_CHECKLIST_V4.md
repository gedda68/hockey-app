...
- [ ] **P3** **Stripe webhook handler hardening** — Extend `POST /api/webhooks/stripe` to handle `charge.refunded` (flag payment as refunded, optionally revert approval) and `invoice.payment_failed` (for future subscription support). Store raw Stripe event in `stripe_events` collection for replay/audit.
