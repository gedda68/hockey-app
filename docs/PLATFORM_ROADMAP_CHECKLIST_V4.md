...
| Category | Key gaps |
|----------|----------|
| **Security** | ~~`/api/clubs` and `/api/payments` unauthenticated; dev test routes exposed~~ ✅ Fixed (S1–S4). ~~No login rate limiting (S5)~~ ✅ Fixed. ~~Cookie flag audit (S6)~~ ✅ Fixed (`__Host-` prefix + full attribute audit). Remaining: rich-text XSS (S7), API endpoint auth test suite (S8) |
...
| ~~3~~ | ~~**S5–S6**~~ | ~~**Auth hardening (rate limiting + cookie flags)**~~ — ✅ **Shipped 2026-04-22.** Sliding-window in-memory rate limiter (10 req/15 min/IP) on all three auth endpoints; `__Host-session` cookie name in production; full attribute audit confirms HttpOnly/Secure/SameSite Lax/Path=/. Remaining: **S7** (Lexical rich-text server-side DOMPurify before MongoDB write). |
